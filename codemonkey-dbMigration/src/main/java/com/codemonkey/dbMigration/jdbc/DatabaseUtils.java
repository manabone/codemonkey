...
public final class DatabaseUtils {
	...
	/**
	 * Given a jdbc url, this tries to determine the target database and returns
	 * the driver class name as a string.
	 * 
	 * @param url
	 *            jdbc url
	...
	public static DBType databaseType(String url) {
		Validate.isTrue(isNotBlank(url));
	...
	/**
	 * Given a JDBC connection URL, generate a new connection URL to connect
	 * directly to the database server itself (ie: no database specified).
	 * 
	 * @param url
	 *            a JDBC connection URL
	 * @return a new JDBC connection URL to connect directly to the database
	 *         server
	 */
	public static String extractServerUrl(String url) {
		...
		// TODO This next line is pretty ugly, but it works for nearly every
		// postgresql server.
		// If we have to add another exception to this for another database
		// server, then I highly recommend refactoring this to a more elegant
		// solution.
		if (DBType.POSTGRESQL.equals(databaseType(url))) {
			baseUrl += "/postgres";
		}
