...
@Service
public abstract class AbsAppUserServiceImpl<T extends AbsAppUser> extends LogicalServiceImpl<T> implements AbsAppUserService<T> {
	...
	public T buildEntity(JSONObject params, CustomConversionService ccService) {
		...
		T appUser = super.buildEntity(params, ccService);
	...
	public boolean login(String username, String password) {
		...
		T user = findBy("username", username);

		if (user != null) {
			String pass = AbsAppUser.encodePassword(password, user.getSalt());
...
//				throw new AuthError(permission.getUrl());
	...
	@Override
	public T doChangePassword(JSONObject body, CustomConversionService ccService) {
		...
		T user = super.buildEntity(body, ccService);
